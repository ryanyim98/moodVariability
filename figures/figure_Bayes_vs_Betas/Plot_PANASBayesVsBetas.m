...
function Plot_PANASBayesVsBetas(Param)
...
map=brewermap(3,'Set1');
 ...
 load('PANASBayes_vs_Betas_data.mat')
 
if Param=='M'
 
    bar([1],table2array(PANASMu_vs_Betas.Coefficients(2,1)),'barwidth',1,'FaceColor',map(2,:),'EdgeColor',map(2,:))
    hold on
    bar([3],table2array(PANASMu_vs_Betas.Coefficients(3,1)),'barwidth',1,'FaceColor',map(2,:),'EdgeColor',map(2,:))
    hold on
    bar([5],table2array(PANASMu_vs_Betas.Coefficients(4,1)),'barwidth',1,'FaceColor',map(2,:),'EdgeColor',map(2,:))
    hold on
    errorbar([1,3,5],table2array(PANASMu_vs_Betas.Coefficients(2:4,1)),table2array(PANASMu_vs_Betas.Coefficients(2:4,2)),'.k','LineWidth',1.5,'CapSize',1.5)
      text(0.9, 0.045, '*','FontSize', 16)
      plot([0.75,1.25],[0.04,0.04], 'k--','LineWidth', 1.6)
...
elseif Param=='V' 
    ...
    bar([1],table2array(PANASVMu_vs_Betas.Coefficients(2,1)),'barwidth',1,'FaceColor',map(1,:),'EdgeColor',map(1,:))
    hold on
    bar([3],table2array(PANASVMu_vs_Betas.Coefficients(3,1)),'barwidth',1,'FaceColor',map(1,:),'EdgeColor',map(1,:))
    hold on
    bar([5],table2array(PANASVMu_vs_Betas.Coefficients(4,1)),'barwidth',1,'FaceColor',map(1,:),'EdgeColor',map(1,:))
    hold on
    errorbar([1,3,5],table2array(PANASVMu_vs_Betas.Coefficients(2:4,1)),table2array(PANASVMu_vs_Betas.Coefficients(2:4,2)),'.k','LineWidth',1.5,'CapSize',1.5)
...
elseif Param=='S'
    ...
    bar([1],table2array(PANASS_vs_Betas.Coefficients(2,1)),'barwidth',1,'FaceColor',map(3,:),'EdgeColor',map(3,:))
    hold on
    bar([3],table2array(PANASS_vs_Betas.Coefficients(3,1)),'barwidth',1,'FaceColor',map(3,:),'EdgeColor',map(3,:))
    hold on
    bar([5],table2array(PANASS_vs_Betas.Coefficients(4,1)),'barwidth',1,'FaceColor',map(3,:),'EdgeColor',map(3,:))
    hold on
    errorbar([1,3,5],table2array(PANASS_vs_Betas.Coefficients(2:4,1)),table2array(PANASS_vs_Betas.Coefficients(2:4,2)),'.k','LineWidth',1.5,'CapSize',1.5)
       text(0.9, 0.22, '*','FontSize', 16)
       plot([0.75,1.25],[0.2,0.2], 'k--','LineWidth', 1.6)
