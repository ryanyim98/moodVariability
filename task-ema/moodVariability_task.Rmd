...
```{r warning=FALSE,message=FALSE}
# source(here::here("./scripts/run_var1model.R"))
...
subj_exclude_novariance_ <- subj_exclude_novariance_rl %>% 
  ...
  count() %>% 
  filter(n>1) #both blocks were constant
...
df_master <- df_master %>% 
  mutate(across(c(AR_PA_sum, AR_NA_sum, AR_PAminusNA_sum,
                  AR_day1_block1, AR_day1_block2,
                  AR_day2_block1, AR_day2_block2), 
                as.numeric)) %>% 
  mutate(AR_task = rowMeans(
    dplyr::select(., AR_day1_block1, AR_day2_block1), 
    na.rm = TRUE))

names(df_master)

df_master[(is.na(df_master$X1)),]
```
...
```{r}
df_days <- df_days %>% filter(!Participant.Public.ID %in% 
                                subj_exclude_novariance_$Participant.Public.ID)
...
### Additional analysis: model with equal coefficients per reward group (summed predictors)

The authors could also have estimated a model with equal coefficients for each group of three past rewards, by summing each group of three rewards to create three new predictors. This constrains the effect of rewards 1-3, 4-6, and 7-9 to be equal within each group.

```{r}
# Summed-predictor model: binary outcome
chart.Correlation(df_master %>% 
                    dplyr::select(X1_sum,X2_sum,X3_sum))
```

```{r}
l1_sum <- lmer(value ~ regression_coef + (1|Participant.Public.ID), df_Xs_scaled_sum_all_long %>%
            mutate(regression_coef = factor(regression_coef, levels = c("X1_sum","X2_sum","X3_sum"), labels = c("trial -1~3","trial -4~6","trial -7~9"))))
summary(l1_sum)
round(confint(l1_sum, level=0.95),3)

l_sum <- as.data.frame(effects::Effect(c("regression_coef"), l1_sum))
p0_sum <- ggplot(data=l_sum, aes(x = regression_coef, y = fit))+
  geom_jitter(data = df_Xs_scaled_sum_all_long %>%
            mutate(regression_coef = factor(regression_coef, levels = c("X1_sum","X2_sum","X3_sum"), labels = c("trial -1~3","trial -4~6","trial -7~9"))),
                   aes(x = regression_coef, y = value), size = 0.3, color = 'pink', alpha = 0.5)+
  geom_point()+
  geom_errorbar(data=l_sum, aes(x = regression_coef, ymin = lower, ymax = upper), width = 0.2)+
  labs(x = "Time of affective impact", title = "Summed-predictor model (equal coefficients per group)")+
...
ggplot(df_master, aes(x = moodrate_sd_day1_block1, y = moodrate_sd_day2_block1))+
  geom_point(size = 0.5)+
  geom_smooth(method = "lm") +
  stat_poly_eq(use_label(c("R2","p")))+
...
df_gor_Est_tc_sum_sd <- df_gor_Est_tc_sum%>% 
  group_by(run,trial_id) %>% 
  summarise_at(vars(mu_orig,vmu,SD), ~sd(.x,na.rm = T)) %>% 
  pivot_longer(c(mu_orig,vmu,SD), names_to = "parameter", values_to = "sd")
...
p3b <- ggplot(data = df_gor_Est_tc_sum %>% filter(parameter == "mu (original space)"), 
  ...
  geom_line(linewidth = 1)+
  geom_ribbon(aes(x = trial_id, ymin = mean-sd, ymax = mean+sd,fill = run),
              alpha = 0.3, color = NA)+
...
bars2$variable[grepl("moodrate_sd",bars2$variable)] <- "task SE"
bars2$variable[grepl("AR_task",bars2$variable)] <- "task AR"
...
summary(l5<-lm(scale(moodrate_sd) ~ scale(mean_mu) + scale(mean5_vmu) + scale(mean_s),df_master))
summary(l6<-lm(scale(PAminusNA_sd) ~ scale(mean_mu_posminusneg) + scale(mean5_vmu_posminusneg) + scale(mean_s_posminusneg),df_master))
...
summary(lm(scale(moodrate_sd) ~  scale(X1),df_master))
summary(lm(scale(moodrate_sd) ~  scale(X1) + scale(X2),df_master))
summary(lm(scale(moodrate_sd) ~  scale(X1) + scale(X2) + scale(X3),df_master))
...
ggsave("../figures/figure_x1x3_sd_mssd_ar.png",width = 6, height = 7)
```

## 2.2 Task params and baseline questionnaires
```{r}
df_corr <- df_master %>% 
                    ungroup() %>% 
                    dplyr::select(TEPS_sum,HPS_sum,CESD_sum,STAI_TA_sum,STAI_SA_sum,
                                  PAminusNA_mean,PAminusNA_sd,PAminusNA_mssd,AR_PAminusNA_sum,
                                  mean5_vmu_posminusneg,mean5_s_posminusneg,
                                  moodrate_mean,moodrate_sd,moodrate_mssd,AR_task,mean5_vmu,mean5_s) %>% 
  rename(`ESM mean` = PAminusNA_mean,`ESM AR` = AR_PAminusNA_sum, `ESM rmssd` = PAminusNA_mssd,`ESM se`=PAminusNA_sd,
         `ESM vmu` = mean5_vmu_posminusneg, `ESM sd` = mean5_s_posminusneg,
         `task mean` = moodrate_mean,`task AR`= AR_task,`task rmssd` = moodrate_mssd,`task se` = moodrate_sd, `task vmu` = mean5_vmu,
         `task sd`= mean5_s) %>% 
  drop_na()
...
p1 <- ggplot(bars %>% 
         filter(!grepl("Intercept",variable)) %>% 
         mutate(variable = factor(variable, levels = c("trial -1~3","trial -4~6","trial -7~9")),
                reg = factor(reg,levels = c("TEPS","HPS","CESD_sum") )))+
...
s1 <- summary(lm(scale(TEPS_sum) ~ scale(mean5_vmu) + scale(mean5_s), df_master))

s2 <- summary(lm(scale(HPS_sum) ~ scale(mean5_vmu) + scale(mean5_s), df_master))

s3 <- summary(lm(scale(CESD_sum) ~ scale(mean5_vmu) + scale(mean5_s), df_master))
...
```{r}
s1 <- summary(lm(scale(TEPS_sum) ~  scale(mean5_vmu_posminusneg) + scale(mean5_s_posminusneg), df_master))

s2 <- summary(lm(scale(HPS_sum) ~  scale(mean5_vmu_posminusneg) + scale(mean5_s_posminusneg), df_master))

s3 <- summary(lm(scale(CESD_sum) ~ scale(mean5_vmu_posminusneg) + scale(mean5_s_posminusneg), df_master))
...
p3+p2+plot_layout(nrow =3)+
  plot_annotation(tag_levels = "A")
ggsave("../figures/figure_x1x3_qs.png",width = 8, height = 8)
```
...
chart.Correlation(df_master %>% 
                    ...
                    dplyr::select(HPS_sum,TEPS_sum,CESD_sum,PA_NA_corr,
                                  PAminusNA_mean,AR_PAminusNA_sum,PAminusNA_mssd,PAminusNA_sd,
                                  mean5_vmu_posminusneg,mean5_s_posminusneg,
                                  moodrate_mean,AR_task,moodrate_mssd,moodrate_sd,mean5_vmu,mean5_s,X1:X3) %>% 
  rename(`ESM mean` = PAminusNA_mean,`ESM AR` = AR_PAminusNA_sum, `ESM rmssd` = PAminusNA_mssd,`ESM se`=PAminusNA_sd,
...
df_reward_sched <- df_d1b1 %>% 
  dplyr::select(Trial.Index,tree1info,tree2info,
                tree1mag,tree2mag) %>% 
  filter(!is.na(tree1info))%>% 
  unique()

write_csv(df_reward_sched,"../data/for_share/reward_sched.csv")
...
df_rt <-  df_all %>%
  ...
  dplyr::select(Participant.Public.ID,Reaction.Time,Trial.Index)
...
pp <-ggplot(df_prob, aes(x = Trial.Index,y = totalmon,
                     color = day, group = interaction(day,Participant.Public.ID)))+
...
df_prob %>% filter(Trial.Index==120, totalmon < 75.5)
...
write_csv(df_Xs_scaled_sum_all_long,"../data/for_share/df_Xs_scaled_sum_all_long.csv")
...
write_csv(df_PANAS_uniform_response,"../data/for_share/df_PANAS_uniform_response.csv")
...
write_csv(df_gor_Est_long,"../data/for_share/df_gor_Est_long.csv")
...
write_csv(df_wof,"../data/for_share/df_wof.csv")
...
write_csv(df_wof_react,"../data/for_share/df_wof_react.csv")
```