...
source(here::here("run_reg.R")) #X1~X3
...
ggsave("./figures/figure_x1x3_new.png",width = 9, height = 4)
...
summary(lm(scale(mean5_vmu_posminusneg) ~ scale(mean5_vmu) + scale(mean_mu) + scale(mean5_s) + scale(PA_NA_corr), df_master))
...
summary(lm(scale(mean5_s_posminusneg) ~ scale(mean5_vmu) + scale(mean_mu) + scale(mean5_s) + scale(PA_NA_corr), df_master))

summary(lm(scale(mean_mu_posminusneg) ~ scale(mean5_s_posminusneg), df_master))

summary(lm(scale(mean5_s_posminusneg) ~ scale(mean5_s), df_master))
summary(lm(scale(mean5_s_posminusneg) ~ scale(mean_mu), df_master))
summary(lm(scale(mean5_s_posminusneg) ~ scale(mean5_vmu), df_master))
...
p2b <-plot_model(lm(mean5_vmu_posminusneg ~ mean5_vmu + mean_mu + mean5_s, df_master), type = "pred", terms = "mean5_vmu",show.data = T,
           color = vmuColor, fill = vmuColor, dot.size = 0.5)+
  labs(x = "task volatility",
       y = "ESM volatility",
       title = "")

p2c <- plot_model(lm(mean5_s_posminusneg ~ mean5_vmu + mean_mu + mean5_s, df_master), type = "pred", terms = "mean5_s",show.data = T,
...
ggsave("./figures/figure_depend.png",width = 6, height = 7)
```
...
```{r}
summary(lm(scale(TEPS_sum) ~ scale(mean_mu_posminusneg) + scale(mean5_vmu_posminusneg) + scale(mean5_s_posminusneg), df_master))
summary(lm(scale(TEPS_sum) ~ scale(mean_mu) + scale(mean5_vmu) + scale(mean5_s), df_master))

summary(lm(scale(HPS_sum) ~  scale(mean_mu_posminusneg) + scale(mean5_vmu_posminusneg) + scale(mean5_s_posminusneg), df_master))
summary(lm(scale(HPS_sum) ~ scale(mean_mu) + scale(mean5_vmu) + scale(mean5_s), df_master))

summary(lm(scale(CESD_sum) ~  scale(mean_mu_posminusneg) + scale(mean5_vmu_posminusneg) + scale(mean5_s_posminusneg), df_master))
summary(lm(scale(CESD_sum) ~ scale(mean_mu) + scale(mean5_vmu) + scale(mean5_s), df_master))
