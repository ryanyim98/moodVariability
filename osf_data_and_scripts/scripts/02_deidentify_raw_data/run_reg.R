# Purpose: per-subject regressions of task mood rating on the preceding 9 trial-back
# reward outcomes (binary win/loss, signed magnitude, and summed-triplet versions),
# producing df_Xs_scaled_all(2)(_sum)_long and joining the subject-level coefficients
# into df_master. Operates on df_reg/df_master/nsub/sub_list/subj_exclude_novariance_rl,
# all built by read_data_task_ema.Rmd -- no file I/O of its own.
...
for (l in 1:4){
  ...
  for (m in 1:nsub){
    ...
    if(nrow(temp_df)>0){
      ...
      if(length(unique(temp_df$moodrate_scaled)) > 1){
...
# Additional analysis: model with equal coefficients for each group of three past rewards
# (by summing each group of three rewards to create three new predictors)
# Binary outcome - summed predictors
df_Xs_scaled_sum_all <- data.frame()

for (l in 1:4){
  db <- db_list[l]
  df_Xs_scaled_sum <- matrix(NA, nsub, 5)
  colnames(df_Xs_scaled_sum) <- c('day_block','Participant.Public.ID','X1_sum','X2_sum','X3_sum')
  
  for (m in 1:nsub){
    temp_participant_id <- sub_list[m]
    df_Xs_scaled_sum[m,1] <- db
    df_Xs_scaled_sum[m,2] <- temp_participant_id
    
    temp_df = df_reg %>%
      filter(Participant.Public.ID == temp_participant_id, day_block == db,
             Mood.Index!=0) %>%
      mutate(reward_group1 = outcome1b + outcome2b + outcome3b,
             reward_group2 = outcome4b + outcome5b + outcome6b,
             reward_group3 = outcome7b + outcome8b + outcome9b)
    if(nrow(temp_df)>0){
      if(length(unique(temp_df$moodrate_scaled)) > 1){
        temp_mod_sum <- lm(moodrate ~ reward_group1 + reward_group2 + reward_group3, data = temp_df)
        df_coeff_sum = as.data.frame(temp_mod_sum$coefficients)
        df_Xs_scaled_sum[m, 3:5] <- df_coeff_sum[2:4, 1]
      }
    }
  }
  
  df_Xs_scaled_sum <- as.data.frame(df_Xs_scaled_sum)
  df_Xs_scaled_sum[, 3:ncol(df_Xs_scaled_sum)] <- sapply(df_Xs_scaled_sum[, 3:ncol(df_Xs_scaled_sum)], as.numeric)
  df_Xs_scaled_sum_all <- rbind(df_Xs_scaled_sum_all, df_Xs_scaled_sum)
}

df_Xs_scaled_sum_all <- df_Xs_scaled_sum_all %>%
  filter(day_block %in% c("day1_block1","day2_block1")) %>%
  group_by(Participant.Public.ID) %>%
  summarise(X1_sum = mean(X1_sum, na.rm = T),
            X2_sum = mean(X2_sum, na.rm = T),
            X3_sum = mean(X3_sum, na.rm = T))

df_Xs_scaled_sum_all_long <- df_Xs_scaled_sum_all %>%
  pivot_longer(X1_sum:X3_sum, names_to = "regression_coef", values_to = "value")

df_master <- left_join(df_master, df_Xs_scaled_sum_all %>% rename(Prolific.Id = Participant.Public.ID), by = "Prolific.Id")
