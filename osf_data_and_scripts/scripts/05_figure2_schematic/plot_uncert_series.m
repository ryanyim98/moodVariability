function plot_uncert_series()
    % Purpose: regenerates fig1c.png (an example participant's filter fit, showing
    % actual data + mean/volatility/noise estimates across 5 sub-blocks with
    % different generative vmu/s regimes) from a cached model fit.
    % Inputs (same folder): 7_bayes_mod.mat, out.mat.
    % Requires the external `makefigure` and `export_fig` toolboxes (see
    % figure_2_main_psci.m for where to point their addpath).
    ...
    load(fullfile(script_dir, '7_bayes_mod.mat'))
    load(fullfile(script_dir, 'out.mat'))
...
%     text(301, -5.5,'high vmu high s','FontSize',8)
    ...
    export_fig(fullfile(script_dir, 'fig1c.png'),'-r 300');
